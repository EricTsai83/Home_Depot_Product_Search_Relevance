...
@brief: 1. spelling checker
        2. I have no time to write a notebook
        3. Actually, PeterNovigSpellingChecker is released. link:  https://pypi.org/project/pyspellchecker/
        4. not used in final submission
"""
...
class DictSpellingChecker:
    ...
    def correct(self, query):
        return self.spelling_checker_dict.get(query, query)   # dict.get(key, default=None) 找字典中的key,並返回None
...
## 沒有表現很好，且我覺得寫的不是很好懂，可以考慮用 https://www.kaggle.com/hiendang/auto-correct-query 替代
class AutoSpellingChecker:
    """
    https://www.kaggle.com/hiendang/crowdflower-search-relevance/auto-correct-query
    
    It tries to find replacement for words from search term which are not found in
    product title (in this paragraph we name them 'misspelled' words). First, the
    algorithm checks whether the misspelled word is actually a combination of two
    separate words. Second, it searches for a good replacement in product titles that
    are matched to queries with the misspelled words. Finally, all product title is
    used as a vocabulary to find an appropriate replacement.
    """
